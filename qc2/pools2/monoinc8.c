/* c:\qc2\transltr\monoinc8.c

   void  p_error( char *error )
   void  displaym()
      scherm2();
      pri_lig( & mm );  in welk file ???
      scherm3();
...
void displaym()
{
    ...
    do {
       print_at(20,20," test matrix-case <y/n> :");
       dis_t= getchar();
       if (dis_t == '#' ) exit(1);
    }
       while (dis_t != 'n' && dis_t != 'y');
    ...
    for (dis_i=0; dis_i< 300 ; dis_i++){
	 dis_mm = matrix[dis_i];
	 pri_lig( & dis_mm );
	 if (dis_t == 'y'){
	    /* print_at(24,10,"units ="); */
	    printf(" %2d",dis_mm.w );
	    if (getchar()=='#') exit(1);
	 };
    }
    ...
    /*
    print_at(24,10,"Nu hoor ik dit te zien ");
    if (getchar()=='#') exit(1);
     */

    do {
       print_at(24,10," einde display <y> stop <#> :");
       dis_t= getchar();
       if (dis_t == '#' ) exit(1);
    }
       while (dis_t != 'y');
