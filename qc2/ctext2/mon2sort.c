...
void zenden()
{
     ...
     BEGIN
	  WRITE('Code: ',TELLER);
	  IF LETTER = $4C THEN REGEL := REGEL + 1;
	  WRITELN(' Regel: ',REGEL,'   ');
     END;
     PORT[POORT] := LETTER;                                    {STUUR KARAKTER NAAR POORT}
     BUSYUIT; { Als BUSY staat mogen we geen STB geven             }
     PORT[POORT+2] := PORT[POORT+2] OR $01;                    {GEEF STROBE              }
     BUSYAAN; { Zien dat STB door interface is ontvangen }
     PORT[POORT+2] := PORT[POORT+2] AND $FE;                   {ZET STROBE UIT           }
     BUSYUIT; { Wacht tot het teken door de interface is verwerkt }
      ...
      */
	   ...
	   */
	   ...
	   /*
	   { for (ziii=0,zjj=0;ziii<10000;ziii++){
	       zjj = zjj+1; zjj=zjj-1;
	   }
	   */
}
...
void testk()
{
  ...
  for (tk =0; tk < 8; tk ++) {
     mcx[0]=bits[tk];   /* tst[tk]; */
		       /* mcx[0] |= 0x80; */
     mcx[1]= 0; /* bits[tk];  tst[tk]; */
     mcx[2]= 0; /* bits[tk];  tst[tk]; */
     mcx[3]= 0; /* bits[tk];  tst[tk]; */
   ...
   do {
	  ...
	  switch ( lbuf[0] ) {
	     ...
	     case '1' :
		  ...
		  break;
	     case '2' :  mcx[2] |= 0x20; bitnr =18;
		  break;
	     case '3' :  mcx[2] |= 0x10; bitnr =19;
		  break;
	     case '4' :  mcx[2] |= 0x08; bitnr =20;
		  break;
	     case '5' :  mcx[2] |= 0x04; bitnr =21;
		  break;
	     case '6' :  mcx[2] |= 0x02; bitnr =22;
		  break;
	     case '7' :  mcx[2] |= 0x01; bitnr =23;
		  break;
	     case '8' :  mcx[3] |= 0x80; bitnr =24;
		  break;
	     case '9' :  mcx[3] |= 0x40; bitnr =25;
		  break;
	     case '0' :
		  switch (lbuf[1] )  {
		     case '7' :  mcx[1] |= 0x04; bitnr = 13;
			break;
		     case '0' :  mcx[3] |= 0x01; bitnr =31;
			break;
		     default :   bitnr = -1;
			break;
		  }
      ...
      if ( bitnr != -1 )  {
	  /* 8 maal dezelfde ventiel

	   */
	   printf(" bitnr = %3d ",bitnr);
	   dispcode();
	   for (ti=0; ti< 8; ti++) {
	      zenden();
	      getchar();
