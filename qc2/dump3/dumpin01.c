...
void f1()
{
    ...
    */
    ...
    if (interf_aan ) {
       ...
       ontcijfer();
       /*
       printf("\ng/k = %2d/%2d var-space %2d/%2d  line %4d \n",
       p0075,p0005, pvar75, pvar05, regelnr+1);
       printf("\nline %4d \n",regelnr+1);
       */
    }
    else {
       ...
       if ( test_NJ() ) {
	  /* printf("NJ=true p0005 = %2d ", */
	  p0005 = test_row();
       }
       if ( test_k() ) p0005 = test_row();

       if ( test_NK() ) {
	  /* printf("NK=true p0075 = %2d ", */
	  p0075 = test_row();
       }
       if (test_g()) p0075 = test_row();
       printf(" g/k = %2d/%2d ", p0075,p0005);
       printf("var-space %2d/%2d", pvar75,pvar05);
       printf(" line %4d ", regelnr+1);
...
char code_min2[4];
char code_min1[4];
char code_75[4]= {0x48, 0x24, 0x0, 0x0  };
char code_05[4]= {0x44, 0x20, 0x0, 0x01 };
...
void hexdump()
{
    ...
    cls();
    print_at(3,1,"hexdump: reading files from disk \n\n");
    ...
    */
		 ...
		 */
		  ...
		  if ( width_row15 != 18 ) {
			 ...
			 icorr -= 15;
		      ...
		      als wiggen fout liggen:
			  wiggen verleggen

		      if ( (pos05 + 15*pos75) != (w05 + 15*w75) ){
			  aanpassen geheugen wiggen
     ...
     */
		...
		/* set file pointer rigth */
		   ...
		   code_75[2]= mcx[2];
		   code_75[3]= mcx[3]; /*{0x48,0x24,0x0,0x0 };*/
...
void cases2()
{
    ...
    */
    ...
    do
    {
	   ...
	   fwidth = get__float(8,33);

	   iset = (int) (fwidth * 2 + 0.5);
	   fwidth = ( (float) iset ) *.5;
	   ...
	   while ( ! ( fwidth >= 3 && fwidth <= 26 ) );

       /* calculate correction */

       u1 = 2;
       u2 = 7;
       printf("Width = %5.2f units wedge[%2d] = %3d ",fwidth,row+1,wedge[row]);

       if ( fabs( ( (float) wedge[row] - fwidth ) ) > .01 )
       {
	   inch = ( ( fwidth - ( (float) wedge[row])) * set) / 1296;
	   /*
