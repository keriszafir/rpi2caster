...
void f0()
{
    ...
    */
    ...
    /* printf("Pos = %5ld ",pos+1); */
    if (interf_aan ) {
       printf("code   %6ld    \n",pos+1);
      /*

       printf("pos %5ld line %3d g/k %2d/%2d var %2d/%2d \n",
	    pos+1,
	    regelnr+1,
	    pos75,
	    pos05,
...
void f1()
{
    ...
    */
    ...
    /* printf("Pos = %5ld ",pos+1); */
    if (interf_aan ) {
       /* printf("Pos %5ld g/k = %2d/%2d line %4d ",
	   pos+1,g ,k ,line+1);
	*/
      /*

       printf("pos %5ld ",pos+1);
       printf(" line %3d ",regelnr+1);
       printf(" g/k %2d/%2d ",pos75,pos05);
       printf(" var %2d/%2d \n",pvar75 ,pvar05);
       */
       ontcijfer();
    }
...
void hexdump()
{
    ...
    number = length / 5;
    printf("\n\nFile length of %s is: %ld number = %5ld \n\n",
    ...
    */
	...
	if (interf_aan ) {
	   printf("Pos %5ld g/k %2d/%2d line %3d \n",
	     pos+1,p0075,p0005,regelnr+1);
	}
	f1();
	zenden_codes();
	...
	if (interf_aan ) {
	   printf("hexdump2 Pos %5ld g/k %2d/%2d line %3d \n",
	     pos+1,p0075,p0005,regelnr+1);
	}
	f1();
	zenden_codes();
	...
	mono.separator = mbuffer[4];
	/*
       printf("pos = %5ld line %3d g/k %2d/%2d var %2d/%2d %2d/%2d n",
	    pos+1,
	    regelnr+1,
	    pos75,
	    pos05,
	...
	if ( ! test_NJ() ) {
	    ...
	    if (test_NK() ) {  /* line to galley both are true .... */
	       ...
	       code_05[1]= 0x20;
	       code_05[2]= mcx[2];
	       code_05[3]= mcx[3]; /*{0x44,0x20,0x0,0x01 };*/
...
void cases2()
{
    double corps_char, width, fb;
    char once, oncel, cc,c1;
    int i,j, number,b,ib, iinch,dx;
    ...
    */
    ...
    cls();
    printf("Casting for cases based on the 5-wedge = cases2 \n\n");
    getchar();
    ...
    do
    {
       ...
       {
	   inch = width ;
	   inch -= (float) wedge[row] ;
	   inch *= set;
	   inch /= 1296;
	   /*
	   inch = (  ( ( width - wedge[row] )* set)) / 1296;
	    */
...
void thin_spaces()
{
	...
	*/
    ...
    do {
       do {
	  /* lees dikte spatie */
	  /* kies plaats om te gieten */
	  /* bereken correctie */
	  /* vraag aantal te gieten spaties */
	  do {
	     print_at(3,1,"how many characters                           ");
	     b = get__line(3,33);
	     number = p_atoi(b);
	  }
	     while (number < 10 || number > 300 );
	  ...
	  /* vraag klaar met deze spatie */
	  do {
	     printf("Nog meer van deze spatie ? ");
	     answer = getchar();
	     switch (answer) {
		case '#' : exit(1); break;
		case 'Y' :
		case 'y' :
		case 'J' :
		case 'j' : answer = 'j'; break;
		default  : answer = 'n'; break;
	     }
	  }
	     while ( answer != 'j' || answer != 'n' );
	  klaar_spatie = ( answer == 'n' );
       }
	  while ( ! klaar_spatie );
       /* vraag klaar met alle spaties */
       do {
	  printf("Nog andere spaties ? ");
	  answer = getchar();
	  switch (answer) {
	     case '#' : exit(1); break;
	     case 'Y' :
    ...
    */
    ...
    do {
       ...
       */
       ...
       width = 0;
       do {
	  print_at(8,1,"Width of space in units < 1-18 >   ");
	  w_sp = get__float(8,33);
	  /* w_sp = width_space; */


	  print_at(10,1,"width = ");
	  printf("%8.2f ",w_sp);
	  width_space = w_sp * 4 + .5;
	  w_i = (int) width_space;
	  width_space = ( (float) w_i ) *.25;
	...
	*/
	  ...
	  for (i=0;i<4;i++) mcx[i]=galley_out[i] ;
	  f0();   /*  line to galley + adjust 0005 wedge position */
	  if ( interf_aan ) zenden_codes();
	  ...
	  if (oncel == 0 ) { /* heating the mould at start */
       ...
       /* cast a little space to prevent the mould to heat too much */
	     ...
	     setrow(u1);
	     f0();        /* pump on + adjust 0075-wedge position */
	     if ( interf_aan ) zenden_codes();
	     /* vierkant vooraf */
	     for (i=0;i<4;i++) mcx[i]=0;
	     f0();        /* square  */
