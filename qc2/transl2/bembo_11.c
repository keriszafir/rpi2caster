...
#include <c:\qc2\transl2\mon_bas1.c>
...
#include <c:\qc2\transl2\bembo011.c>
	/*
...
#include <c:\qc2\transl2\monoinc7.c>  /* intro intro1 */
#include <c:\qc2\transl2\monoinc8.c>
		 /*  zoek() search routine in diecase    */
#include <c:\qc2\transl2\moninc10.c>
#include <c:\qc2\transl2\monolus.c>
...
void test_tsted( void )
{
	...
	if ( EOF_f == 0 )
	{
	   disp_line();
	   tst_used = testzoek5 ( ) ;
	   printf("number lines %4d nrb %3d chars used %3d ",
		      ++aantal_lines, nreadbuffer, tst_used );
...
#include <c:\qc2\transl2\monoine3.c>
/* testzoek5 */
...
main()
{
    ...
    do {
       print_at(1,0,"");
       printf(" druk een toets in :");
       testkey ();
       print_at(3,0,"");
       printf(" asc = %2d key1 = %3d key = %3d ",asc, key1, key);
    }
       while ( key != '#'  );
     */
