...
printf("         *********************************************************\n");
prleeg();
printf("         *          Dump : code transferring program             *\n");
prleeg();
printf("         *         version 1.1.15 date 12 December 2011          *\n");
prleeg();
printf("         *          to keyboard or composition caster            *\n");
prleeg();
printf("         *              for large-composition                    *\n");
prleeg();
printf("         *                   program by:                         *\n");
prleeg();
...
/*  26 feb 2006:
    adjustment for wedges with other than 9 units in the 5th row
    asking number of units in the 5th row
    program will display the value in inches
...
void aline_caster()
{
    ...
    char col[2] , row[2];
    unsigned b[4];
    double units9, units18, difff;
    int  i, n_inch, nwidth, bb, number5;
    double w_row5;
    ...
    printf("  9 units = %7.4f   18 units = %7.4f \n",units9, 2*units9 );
    ...
    /*
       read number of units in row 5 :
	   > = 5 && <=10
       */

    do {
       print_at(10,1,"Number of units in row 5 :");
       number5 = get__int(10,33);
    }
       while (number5 < 5 || number5 > 12 );
       w_row5 = set * number5 / 1296 ;
    ...
    do
    {
       ...
       printf("   %2d units %6.2f set = %7.4f \n",number5, set, w_row5);
       printf("   %2d units %6.2f set = %7.4f \n",number5*2, set, w_row5*2);
       printf("\n");
       printf("ready ? <y/n> ");
       while ( !kbhit() );
       cont = getche();
    ...
    n_inch = nwidth * set / 1296.;

    u1=3;
    u2=8;
    if ( wedge[rnr] != nwidth )
    {
	 difff = ( (float) ( ( nwidth - wedge[rnr] )* set)) / 1296;
	 adjust(difff);
	 l[1]+= 0x20; /* S aan */
    }
    ...
    setrow(u2);
    for (tj=0;tj<4;tj++)galley_out[tj] =mcx[tj] ;

    mcx[0]=0; mcx[1]=0x00;  /* 0x20;   S */
    mcx[2]=0; mcx[3]=0x01;  /* 0005 */
    setrow(u2);
    for (tj=0;tj<4;tj++) cancellor[tj] = mcx[tj] ;

    mcx[0]=0; mcx[1]=0x04; /* g 0x24;  S + 0075 */
    mcx[2]=0; mcx[3]=0x00;
    setrow(u1);
    for (tj=0;tj<4;tj++) pump_on[tj] = mcx[tj] ;
    ...
    */
    ...
    for (tj=0;tj<4;tj++) mcx[tj] = cancellor[tj];
    ...
    for (tj=0;tj<4;tj++) mcx[tj]=pump_on[tj];
    ...
    do
    {
	  ...
	  /* 0005 => 8 position */
       ...
       for (tj=0;tj<4;tj++) mcx[tj]=galley_out[tj];
       f1();
       if ( interf_aan ) zenden_codes();  /* eject line */
       f1();
	...
	*/
    ...
    for (tj=0;tj<4;tj++) cancellor[tj]=0;
    /* mcx[3] = 0x81; */ /* pump off */
    f1();
    if ( interf_aan ) zenden_codes();
    if ( interf_aan ) zenden_codes();
