/*
       ...
       bembo 16 romein/italic 2 juni 2005



       MONOTYPE program

       central in      monobai1.c
  ...
  info testzoek5  in file : c:\qc2\bembo17\monoin00.c
       lus:       monolus
  gen_system in:  bmgar_06.c
		  bmbit_06

  #include <c:\qc2\bembo17\gar_it01.c>  * : wig [] 536... *


  #include <c:\qc2\bembo17\bmgar_06.c> * gegevens matrijzenraam *
  ...
  calc_maxbreedte in hoofdprogramma

  gen_system in: bmbrm_06
		 bmbit_06
...
int li1, li2; /* uitvulling berekend in lus */
float uspace; /* dikte uitvul spatie */

int wedge_set = 52;    /* 13 set 1331 wig */
int      u1,u2;
unsigned char l[4];

void correction( int w, int rij );
char test_zwart( char c);
...
float _inch_over, _units_over , _over;
int _iover;
...
#include <c:\qc2\bembo17\gar_it01.c> /* : wig [] .. */
...
#include <c:\qc2\bembo17\bmgar_06.c>
#include <c:\qc2\bembo17\monoinc7.c>  /* intro intro1 */
#include <c:\qc2\bembo17\monoinc8.c>
#include <c:\qc2\bembo17\moninc10.c>
#include <c:\qc2\bembo17\monolus.c>
...
void test_tsted( void )
{
    ...
    regel_start = 1;  /* the line will start */
...
struct   monocode  coderec;     / * file record code file * /
	...
	if ( EOF_f == 0 )
	      leesregel(  HALFBUFF - nreadbuffer - 3 );
	...
	while ( nreadbuffer > 0 ) {
	   /*
	      begin van de regel
	      waarden van de regel resetten....

	    */

	   if (regel_start == 1 ) {
	       /* regel waarden op default zetten */


	       clear_lined();
	       if ( central.cset <= 36 ) {
		 /*
		    dit moet aan het begin van een regel
		 ...
		 if (caster == 'k' ) { /* naar interface */
	       ...
	       }   /* dit moet aan het begin van de regel */
	       lnum = 0;
	       if (line_data.vs > 0 ) {
	   ...
	   if (nreadbuffer > tst_used ) { /* move rest buffer to the front */
	      ...
	      printf(" nr = %3d ",nreadbuffer);

	      for (tst_i = -2; tst_i < 5 ; tst_i ++)
		  printf(" %3d %1c ",tst_i ,
		       readbuffer[tst_i + tst_used ]);


	      print_at(6,1,"");

	      for (tst_i = 0; tst_i <nreadbuffer - tst_used; tst_i++) {
		  readbuffer [tst_i] = readbuffer[tst_i + tst_used ];
		  readbuffer2[tst_i] = readbuffer[tst_i];

		  printf("%1c",readbuffer[tst_i]);
	      }
	      ...
	      for (tst_i = nreadbuffer - tst_used;
			  tst_i< nreadbuffer; tst_i++)
		  readbuffer[tst_i] = '\0';
		 ...
		 printf("%1c",readbuffer[tst_i]);
	      ...
	      print_at(22,1,"verschoven =>");
	      printf("nb =%4d ",nreadbuffer);
	   ...
	   } else { /* wat doen als tst_used == nreadbuffer */
	...
	while ( ! feof (fintext ) ) {
	   ...
	   if (line_data.vs > 0 ) {  /* margin .... */
		 ...
		 line_data.last = central.inchwidth;
	      ...
	      tst2_int = ( int ) line_data.wsum * 5184. / central.cset ;
...
void leesregel(int aantal )
{
   ...
   for (crp_i=0;  crp_i < aantal
		  && crp_i < HALFBUFF-3
		  && (crp_l = lees_txt( filewijzer )) != EOF
		  && ! eol
		  ; crp_i++ ){
	   ...
	   switch ( crp_l ) {
		 ...
		 switch ( crp_l) {
			...
			crp_fread++;
			crp_i++;
			if (crp_l == 'F') eol++;  /* Flat filled */
			if (crp_l == 'R') eol++;  /* right filled */
			if (crp_l == 'L') eol++;  /* left filled */
			if (crp_l == 'C') eol++;  /* centered */
			if (crp_l == 'J') eol++;  /* ???? */
			break;
		    case 'E' :
			eol++;
...
{
	/*
	if ( EOF_f ) {   dit kan niet goed zijn.....
	   nreadbuffer = 3;
	   readbuffer[3] = '\0';
	}
	 */
...
void calc_maxbreedte ( void )
{
   ...
   if (central.wset <= 48 ) {
      maxbreedte +=
      ( (float) (line_data.nspaces*2-6))*((float) central.wset)/5184.;
   } else {
      maxbreedte +=
      ( (float) (line_data.nspaces  -6))*((float) central.wset)/5184.;
   }
   if (line_data.rs > 0 )
...
#include <c:\qc2\bembo17\testzoek.c> /*  monoin00.c> */
...
void correction( int w, int rij )
{
     ...
     if (dx < 0 ) {
	 printf("correction incomplete :\n");
	 printf("char = %2d units %8.3f set\n",w,char_set );
	 printf("rij  = %2d units %8.3f set\n",wig[rij],wedge_set);
	 printf("width row [%2d] = %10.5f \n",rij,dikte_rij);
	 printf("width char      = %10.5f \n",dikte_char);
	 printf("correction needed %10.5f \n",dikte_char-dikte_rij);
	 if (getchar()=='#') exit(1);
	 dx = 0;
     }
     if (dx > 224 ) {
...
main()
{
    ...
    */
    ...
    cls();
    printf("Monotype program Garamond 5 jan 2005 \n\n\n");
    printf("version 5 jan 2005 ");
    getchar();
    */
    ...
    do {
	  ...
	  bb2 += .125 ;
	  bb1 = (int) bb2 ;
	  printf(" bb2 = %10.2f bb1 = %4d ",bb2,bb1);
	  bb2 = (float) bb1;
	  bb2 *= .25;
	  bb3 = ( (float) bbb )* 230.1696 / bb2;
	  printf("bb3 = %10.3f ",bb3);
	  if (getchar()=='#')exit(1);
	  central.set = bb1;
	  central.rwidth = (float) bbb;
	  central.inchwidth = ((float) bbb) * .1776;
	  central.lwidth = (int) bb3;
       */
