...
int  lus( char ctest )
{
   ...
   switch ( ctest )
   {
	 ...
	 switch ( luscy )   /* readbuffer[t3j+1] */
	 {
	    ...
	    case '#' :  /* @#n add n times 18 units squares alpha-hex .... */
	       ...
	       if ( ( line_data.wsum +
		    ( (float) (alphahex(luscx) * 18 * central.cset) ) / 5184.)
		      > central.inchwidth ) {
	    ...
	    case '=' :  /* @=n -> add n half squares alphahex... */
	       ...
	       if ( ( line_data.wsum +
		      ((float) ( alphahex(luscx) * 9 * central.wset ) ) / 5184.)
		      > central.inchwidth ) {
	 ...
	 plaats[lnum] = t3j;
	 plkind[lnum] = kind;
	 /* berekenen uitvulling die de regel zou uitvullen */
	 ...
	 if (line_data.para == 0) {
	    ...
	    line_data.wsum +=
		  (datafix.wunits * (float) central.wset ) / 5184.;
	    line_data.nfix ++;
   ...
   /* wegschrijven gegenereerde code */
