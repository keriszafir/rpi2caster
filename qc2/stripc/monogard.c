/*
       ...
       MONO-gard X   27 juni 2004

       monogard
       MONOTYPE program

  matmax in: monoinc5.c...

	      matmax :    monogara5
       info diecase in:
	      #include <c:\qc2\stripc\monogar1.c> : wig [] 536...
	      #include <c:\qc2\stripc\monogar6.c> * gegevens matrijzenmcraam *


  info wedge  in file :   c:\qc2\stripc\mONOInc1.C
  info linewidth      :
  info matrix in file :  <c:\qc2\stripc\monoinc6.c>
		  tijdelijk:            monoin26.c

		  lus : monoincl9

  fixed_space () : monoinc6.c
...
void gotoXY ( int r, int y )
{
   _settextposition( (short) y, (short) r );
			 ...
			 /* inc1 = monogar1 */
#include <c:\qc2\stripc\monogar5.c>
...
void dispcode(unsigned char letter[])
{
    ...
    converteer (letter);
		      ...
		      /*monoinc6 */
#include <c:\qc2\stripc\monogar6.c>
...
struct   monocode  coderec;     / * file record code file * /
    ...
    do {
	printf("zetbreedte (aug) ");
	get_line();
	if (line_buffer[0]=='#') exit(1);
	tst2width = atof(line_buffer);
    }
	while ( tst2width < 8 || tst2width >40 );

    ask_set();



    central.inchwidth = tst2width * .1776;

    central.rwidth    = tst2width;      /* width in cicero     */

    central.inchwidth = tst2width*.1776;   /* width in line in inches    */
    central. lwidth   =
	  (int) ( .5 + ( tst2width* 920.6784 /
		 (float) central.set ) ) ;  /* width of the line in units */
  ...
  */
    ...
    printf(" %8.2f D",central.rwidth );
    printf(" %8.4f\" ",central.inchwidth );
    printf(" %5d u= # ", central.lwidth);
    if (getchar()=='#') exit(1);
  ...
  /*
	...
	while ( nreadbuffer > 0 && ! stoppen ) {
	    ...
	    nreadb3 = 0;
	    print_at(12,1,"");
	    printf("used %3d nreadbuffer %3d",tst_used,nreadbuffer);
	    ce();
...
void test2_tsted( void )
{
    ...
    /* printf( "input file name: " );*/ /* gets( inpathtext ); */
	...
	printf( "File %s is: %ld byte", outpathtmp, lengthtmp );
...
void crap ()
{
    ...
    printf("zet pointer 10 terug"); ce();
    filewijzer -= 10;
    ...
    printf("l string %3d ",crp_l = strlen(readbuffer));
