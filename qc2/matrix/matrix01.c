...
struct matrijs far matrix   [ MATMAX ];

char line_buffer[80];

char name[25]="Dante 592              ";
...
typedef struct naamrecord {
     ...
     unsigned char wedge[16];    /* wedge */
};

struct naamrecord naam1,     /* input file */
		  naam2,     /* output file */
...
#include <c:\qc2\matrix\get__rtn.c>
...
/*
#include <c:\qc2\matrix\monofnt2.c>
 */
...
void kiesfont()
{
     ...
     do {
	 cls();
	 printf("                Choose standard diecases\n\n");
     /*  printf("                    baskerville 14             = B   \n");
	 printf("                    bembo                      = b   \n");
	 printf("                    caslon 12                  = C   \n");
	 printf("                    cochin                     = c   \n");
	 printf("                    dante             592      = d   \n");
	 ...
	 printf("                    ehrhart                    = e   \n");
     */
	 printf("                    garamond          156/174  = g   \n");
	 printf("             11     times new roman   237      = t   \n\n");
	 printf("                                         font  = ");
	 while ( ! kbhit());
	 c=getche();
     }
     ...
     for (i=0;i<MATMAX;i++){
	 switch ( c ) {
     /*     case 'B' : matrix[i]=bask14[i];   break;
	    case 'b' : matrix[i]=bembo[i];    break;
	    case 'C' : matrix[i]=caslon12[i]; break;
	    case 'c' : matrix[i]=cochin[i];   break;
	    case 'd' : matrix[i]=dante[i];    break;

	    case 'e' : matrix[i]=ehrhart[i];  break;
      */
	    case 'g' : matrix[i]=garamond[i]; break;
	    default  :
	    /* case 't' : */ matrix[i]=times11[i];  break;
...
void lees_naam()
{
   ...
   cls();
   print_at(4,5,"Name of this face =                       ");
   /*       123456789012345678901234567890 */
   length = get_line_at(4,26,25);

   for (i =0;i<25;i++) name[i]='\0';

   for ( i=0; (i<25) && (i<length) && ( line_buffer[i] != '\0') ; i++ ) {
	name [i] = line_buffer[i];
	naam2.naam_str[i] = line_buffer[i];
   }
   while (!kbhit());
...
void menu()
{
    ...
    do {
	 ...
	 case 'n' :
	     ...
	     /* wis diecase */
	     /* read all places */
	     /* display the mats after reading */
	     break;
	 case 'r' :
	     read_file();
	     break;
...
{
     if (m.mrij != -1 ) {
	print_at( m.mrij+4,m.mkolom*4 +8,"");
	switch ( m.srt){
...
void disp_matrix()
{
   ...
   for (i=0;i<RIJAANTAL-1;i++) printf("   %2d\n", wig[i]);
...
void hex_main()
{
    ...
    */
    ...
    {
	/*
	printf( "Enter record between 1 and %3d (or 0 to quit): ",MATMAX );
	scanf( "%d", &newrec );
	*/
	cls();
