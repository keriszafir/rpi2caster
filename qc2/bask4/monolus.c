   ...
   switch ( ctest )
   {
	 ...
	 switch ( luscy )
	 {
	    ...
	    case '#' :  /* @#n add n times 18 units squares alpha-hex .... */
		  ...
		  if (lusaddsqrs != 0 ) {
			...
			/*
			   ...
			   and maxbreedte should not be reached...
			   !!!!!!!!!!!!!
	    ...
	    case 'E' :  /* EOF reached */
	       ...
	       t3j++;
	       break;
	    case 'F' : /* fixed spaces = 3 + n * .25  points */
	    ...
	    case 'w' :
	       ...
	       central.rwidth   += (readbuffer[t3j+3] - '0');
	       t3j++;
	       break;
	    case 's' :
	       central.set  = alphahex( luscx ) * 4;
	       central.set += (readbuffer[t3j+3] -'0') ;
	       t3j++;
	       break;

	    case 'B':
	       switch ( readbuffer[t3j+2] ){
		  case 'P' :
		  case 'p' :
