...
/* File record
    ...
    lig = 4 char : to accomodate the unicode standard

*/

typedef struct matrijs {
     char lig[4];            /* letter inhoud matrijs */
...
char r_eading()
{
    ...
    size_t recsize = sizeof( matfilerec );
    ...
    do {
       ...
       print_at(13,10,"Enter input file name:   " ); gets( inpathmatrix );
    ...
    handle = open( inpathmatrix,O_BINARY |O_RDONLY );
    ...
    *fp = ( fpos_t ) ( p ) ;
    ...
    for (recseek = 10; recseek <= aantal_records -11;
	    ...
	    *fp = ( fpos_t ) ( p ) ;
	    fsetpos( finmatrix , fp );
	    fread( &matfilerec, recsize, 1, finmatrix );
...
int schrijf()
{
	...
	gets( outpath );
	if( ( fpoutmatrix = fopen( outpath, "wb+" )) == NULL )
	{
	...
	fwrite( &cdata , recs2, 1, fpoutmatrix );
					/* next the array */
	for( i = 0; i < 272 ; i++ )
	{

	   for (j=0;j<3;j++)
	       matfilerec.lig[j] = matrix[i].lig[j];
	   matfilerec.srt    = matrix[i].srt ;
	   matfilerec.w      = matrix[i].w ;
	   matfilerec.mrij   = matrix[i].mrij ;
	   matfilerec.mkolom = matrix[i].mkolom;

	   fwrite( &matfilerec, recsize, 1, fpoutmatrix );
	}
	fclose (fpoutmatrix);
	rtvalue = 1;
