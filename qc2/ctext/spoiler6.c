...
struct monocode dicht[204] = {

    0xff,0xff,0xff,0xff,  0xf0, /* stop */
    0x4c,0x04,0,   0x01,  0xf0, /* NJK */
    0x04,0x20,0,   0x20,  0xf0,
    0x48,0x04,0x08,0,     0xf0,
    0x44,0,   0,   0x09,  0xf0,
    0x02,0,   0x20,0,     0xf0,
    0x10,0x20,0,   0x80,  0xf0,
    0x48,0x04,0x20,0,     0xf0,
    0x44,0,   0x08,0x01,  0xf0,
    0x10,0,   0x08,0,     0xf0,
/*10 */
    0x02,0,   0x08,0,     0xf0,
    0x04,0,   0x08,0,     0xf0,
...
/*50*/
    ...
    0x48,0x04,0x08,0,   0xf0,
/*60*/
    0x4c,0x04,0x02,0x01,0xf0,

    /*Uitvergroting van de bruidsluier filtert*/
    0,   0xa0,0,   0x04,0xf0,
    0x48,0x04,0x08,0,   0xf0,
    0x44,0,   0,   0x09,0xf0,
    0,   0x40,0x40,0,   0xf0,
...
/*120*/
    ...
    0,   0xa0,0x04,0,   0xf0,
    0,   0,   0,   0,   0xf0,
    0x48,0x04,0x02,0,   0xf0,
    0x4c,0x04,0,   0x03,0xf0,
    /*de inkijk in het gezinsleven. Land van */
...
void quit( int error );
unsigned char sent_to_interface(unsigned char cc[]);
...
int testbit( unsigned char  c, unsigned char  nr);
...
/* version: 8 january 2003:
   ...
   4 byte string sent to interface
...
{
    int i;
    unsigned char busy_on;
    unsigned char time_out;

    /* timeout variable to built in yet */
    ...
    for (i=0;i<4;i++) {
       outp(SENT_BYTE,cc[i]);
		 /* set data byte ready on port */
       outp(MAKE_STROBE,0xff);
		 /* set STB at 0 => make a strobe
		    a data-byte is ready for reading */
       do {
	  busy_on = inp(READ_STATUS);
		 /* read status port to interface :
		    control the byte is actually read */
	  busy_on &= 0x80;
		 /* isolate busy-bit from status-byte */
	  /* timeout */
       }
	  while ( ( busy_on != 0 ) /* && ( ! timeout) */ );
	  /* busy > 0 : wait until data is read */
	  /* timeout                            */
       /* if ( ! timeout ) { */
	  outp(MAKE_STROBE,0xfe);   /* set STB at 1 => the data-byte
						is removed */
	  do {
	     busy_on = inp(READ_STATUS);
		    /* read status port to interface
	     ...
	     while (busy_on != 0 /* && ( ! timeout ) */ );
	     /* wait until port is capable for output */
	     /* timeout */
...
char interface ( unsigned char  c[] )
{
   ...
   do {
	  ...
	  status = 1;
       ...
       printf("status = %2d timeout = %2d ",status,timeout );
       getchar();



       if ( status ) {
	  printf("status is waar ");
	  getchar();
	  for (i=0;i<=3;i++) {
	      printf("nu gaat bit %2d ",i);
	      getchar();
	     _bios_printer ( _PRINTER_WRITE, LPT1, c[i]);
	  }
	  timeout = 1;
       } else {
	  printf("status  is niet waar ");
	  time( &ltime );
	  delta = ltime - begin;
       }
       ontsnap("ontsnappen interface");

   }
   while ( delta < MAXTIME && timeout != 1 );
   printf("Het is goed gegaan ");
   getchar();
...
void w_codefile()
{
    ...
    unsigned int aantal_records=0;

    /* file text eruit zetten */
    printf( "give proper directions\n\n");
    printf( "Enter output file name for text : " );
    gets( outpath );
    ...
    /* text[ */
     for (i=0;i<29;i++){
	printf("nu regel %3d ",i);
	getchar();
	if( fputs( text[i] , fpout ) == EOF ) {
	    printf("error during file handling ");
	    getchar();
	    exit (1);
	}
     }
    ...
    cls();
     cx[0] =0; cx[1]=0xa0; cx[2]=0x40; cx[3] =0;
...
/* uitvullen
   calculation position adjustment wedges
*/
...
/*
   ...
   uses glabal var:
      ...
      qadd
      left
      var  number of variable spaces to be used
...
void fill_line(int som, int   )
{
     /* alles common
     double zetbreedte; /* line width in inches */
...
{
     int i,j,k;
     unsigned char voorraad[4] = {0xaa,0x55, 0xcc, 0x33};
     unsigned char cxx[4]= {0,0,0,0,};
     ...
     */
     ...
     unsigned int u[28] = {
		   322, 247, 266, 338, 325, 270, 278, 300, 321, 322,
		   222, 279, 250, 309, 316, 257, 252, 240, 259, 278,
		   288, 290, 262, 167,  99, 112, 148
     };
     ...
     aug = 20; /* zetbreedte in aufustijn */
     var = 3;  /* aantal varable spaties bij aanvullen regel */
     cls();
...
void mainx()
{
    ...
    /* mainx hier was ik interface */
     ...
     for (i=0;i<8;i++){
	printf(" i = %2d %4x ",i,testbyte);
	for (j=0;j<8;j++)
	  printf("%1c",testbit(testbyte,j)+'0');
	...
	testbyte*=2;
	 ...
	 for (j=0;j<4;j++)
	    printf(" %2x ",cb[j]);
	 getchar();
	 sent_to_interface (cb);
	 ontsnap("lus na sent to interface ");
     }
       */
   ...
   do {
       do {
	 printf("commando i = init  s = status \n");
	 printf("         b = byte  u = interface \n");
	 printf("         p = strobe \n");
	 printf("         # = stoppen ");
	 get_line();
	 c1x = r_buff[0];
	 ...
	 while (c1x != 'i' &&
	  ...
	  case 'i' :
	     status = _bios_printer(_PRINTER_INIT,LPT1,0);
	     printf("status na init         = %2x ",status);
	     pstatus = status;
	     if( (pstatus & 0x29) || !(pstatus & 0x80) || !(pstatus & 0x10) )
		 pstatus = 0;
	      else
		 pstatus = 1;
	     printf("status %4x pstatus = %2d ",status, pstatus );

	     getchar();
	     break;
	  case 'u' :
	     printf("uitsturen 1 byte ");
	     outp(0x378,cxc);
	     outp(0x37A,0xff);  /* strobe */
	     break;
