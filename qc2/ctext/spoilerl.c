...
int row_test (char c[])
{
   ...
   return (i-16);
    ...
    int zni,znj;
    char zcb[4]; char zcx[4];
    char ze1[4]; char ze2[4];
    char zp1,zp2,zp3,zp4;
    char zln_uitvul[2];
    char zt_u[2];
    char zchar_uitvul[2];
    int zlt, zut, ztut;
...
void zenden(int nr )
{
    ...
    for (zni=0; buff[zni] != -1 ; ) {
	for (znj=0;znj<=3;znj++) {
	   zcx[znj]=buff[zni+4];
	   zcb[znj]=buff[zni++];
	}
	zp1=1; zp2=0;
	zr1 = row_test(zcb); zr0 = row_test(zcx);

	/* printf("%2d/%2d ",zr0,zr1);*/

	if ( (NJ_test ( zcb) + NK_test(zcb)) == 2) {
		 /* printf("Beginning of a line\n");*/
	   zln_uitvul[1] = zr0;
	   zln_uitvul[0] = zr1;
	   zchar_uitvul[0] = zln_uitvul[0];
	   zchar_uitvul[1] = zln_uitvul[1];
	   zp1=1; zp2=1; /* both codes will be needed */
	} else {
	   if ( (NJ_test (zcb) + NK_test(zcx) ) == 2) {
		zt_u[1] = zr0;
		zt_u[0] = zr1;
		ztut = zr0*15+zr1;
		zut = 15*zchar_uitvul[1] + zchar_uitvul[0];
		if (ztut == zut ) {
		   ...
		   /*  printf("wedges out of position:\n");
		       printf("adjustment code is sent  %2d/%2d \n",
			      zt_u[1],zt_u[0]);
		    */
		   zp1=1; zp2=1; /* both to caster */
		}
		zchar_uitvul[0] = zt_u[0];
		zchar_uitvul[1] = zt_u[1];
	   } else {
	      if ( GS2_test(zcb) == 1) {
		 zlt = 15*zln_uitvul[1] + zln_uitvul[0];
		 zut = 15*zchar_uitvul[1] + zchar_uitvul[0];
		 if ( zut != zlt ) {
		    /*
		    ...
		    ze2[0] = NJ; ze2[1]=0;     ze2[2]=0; ze2[3]=0x01;
		    ze1[0] = NK; ze1[1]=W0075; ze1[2]=0; ze1[3]=0x0;

		    setrow( ze2, zln_uitvul[1]-1 );
		    zp3 =1;
		    showbits(ze2);                   /* to -> interface */
		    setrow( ze1, zln_uitvul[0]-1);
		    showbits(ze1);                   /* to -> interface */
		    zp4 = 1;
		    zchar_uitvul[0] = zln_uitvul[0];
		    zchar_uitvul[1] = zln_uitvul[1];
		 } else {
	...
	if (zp1==1)
	    showbits(zcb);         /* to -> interface */
	if (zp2==1) {
	    showbits(zcx); zni += 4; /* to -> interface */
	}
/*        printf("lu %2d/%2d cu %2d/%2d \n",zln_uitvul[0],zln_uitvul[1],
		   zchar_uitvul[1],zchar_uitvul[0]);
	   if ( getchar() =='#') exit(1);
	*/
    }
}  /*  zenden
...
char menu (char fileopen )
{
   ...
   scherm();
   do {
     print_at( 6,32,"menu spoiler-program");

     if (fileopen == 0) {
	print_at(10,33,"open code-file = o");
     } else {
	print_at(10,31,"change code-file = O");
     }
     if (fileopen ) {
...

{
   ...
   do {
      ...
      switch (command ) {
	 case 's' : /* write experimenteel file */
	   w_codefile();
	   break;
