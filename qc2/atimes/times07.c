/*
       ...
       central in      monobas1.c
       wedge   in      monobas1.c


  info testzoek5 in file : c:\qc2\bask\monoin00.c


	      #include <c:\qc2\stripc\monogar1.c> : wig [] 536...
	      #include <c:\qc2\stripc\monoba14.c> * gegevens matrijzenmcraam *
...
#include <c:\qc2\atimes\mon_bas1.c>
/* : wig = 5 wedge  */
...
void calc2_maxbreedte( int inword );
...
#include <c:\qc2\atimes\monoti07.c>

		       /* monoba14 = baskrville 14
...
#include <c:\qc2\atimes\monoinc7.c>  /* intro intro1 */

#include <c:\qc2\atimes\monoinc8.c>
#include <c:\qc2\atimes\moninc10.c>
#include <c:\qc2\atimes\monolus.c>
...
void calc2_maxbreedte( int inword )
{
   ...
   if (central.set <= 48 ) {
      maxbreedte +=      /* 6 =width afbreekteken */
      ( (float) (line_data.nspaces*2-6*inword ))*((float) central.set)/5184.;
   } else {
      maxbreedte +=
      ( (float) (line_data.nspaces  -6*inword ))*((float) central.set)/5184.;
   }
   if (line_data.rs > 0 )
...
void afbreken( void )
{
    ...
    print_at(15,1,"number characters = ");
    printf("%4d ",line_data.line_nr - 1 );
    if (getchar()=='#') exit(1);
...
#include <c:\qc2\atimes\monoin00.c>
...
main()
{
    ...
    */
    ...
    cls();
    printf("Monotype program Times 7 point 327-7\n\n\n");
    printf("version 17 dec 2005 ");
    getchar();
