...
void store_text(char laatst);   /* store output text in file */
void ddd();
...
/********* #include <c:\qc2\pools2\rowcol2.c>   wordt niet gebruikt....
...
/********* #include <c:\qc2\pools2\mon_bas1.c>
...
/**************  #include <c:\qc2\transl2\mococh10.c>   *****************/
...
/****************  #include <c:\qc2\pools2\monoinc7.c>  *****************/
...
/***************   #include <c:\qc2\pools2\monoinc8.c>  *****************/
...
/***************   #include <c:\qc2\pools3\moninc10.c>  *****************/
...
/***************   #include <c:\qc2\pools3\monoluse.c>   *****************/
...
/*
   #include <c:\qc2\pools2\mobask14.c>
   #include <c:\qc2\pools2\dante014.c>
   #include <c:\qc2\pools2\dante012.c>
   #include <c:\qc2\pools2\mococh12.c>



*/

   #include <c:\qc2\pools2\mococh08.c>
/*
   #include <c:\qc2\pools2\mococh10.c>
*/


#include <c:\qc2\pools2\gen_syst.c>
	/*
...
#include <c:\qc2\pools2\monoinc7.c>  /* intro intro1 */
#include <c:\qc2\pools2\monoinc8.c>
...
void store_text(char laatst )
{
   ...
   sti=0;
   /*
     toegevoegde ^ae aan het begin van de regel: geeft 2 "spaties"
     aan het begin van de regel

     de nullen die in de buffer staan, worden genegeerd in EDIT
    */
    /*
   if (laatst == 1 ) {
      printf(" laatste wegschrijven n = %3d ",outb_n);
      if (getchar()=='#') exit(1);
   }
    */
   while ( /* outtextbuffer[sti] != '\0' && */
	     sti < outb_n && sti < BUFSIZ ) {
      strr = 1;
      if ( sti == 0 && outtextbuffer[sti]=='\0' ) strr=0;
      if ( sti == 1 && outtextbuffer[sti]=='\0' ) strr=0;
      if ( sti >  1 && outtextbuffer[sti]=='\0' ) break;

      if (strr) {
	 fputc( outtextbuffer[sti], fouttext );
      }
      sti++;
   }
}
...
#include <c:\qc2\pools3\tsteditE.c>
...
void lus_eerst()
{
   ...
   for (iel=0; iel < nreadbuffer; ) {
      icc = readbuffer[iel];
      switch (icc) {
	 case '^' :
	    iec1 = readbuffer[iel+1];
	    iec2 = readbuffer[iel+2];
	    switch (iec1) {
		...
		/* the two next chars will not be evaluated a second time */
		  ...
		  readbuffer[iel  ] = '\00' ;
		  readbuffer[iel+1] = '\00' ;
		  readbuffer[iel+2] = 16*alphahex( iec1 ) + alphahex( iec2 );
		  break;
	    }
	    iel += 3;
	    break;
...
void leesregel()
{
   ...
   for (crp_i=nreadbuffer  ;  crp_i < HALFBUFF-3 &&
	  ( (crp_l = lees_txt( filewijzer ++ )) != EOF ) && ! eol ;
	   ...
	   switch ( crp_l ) {
	      ...
	      case '^' :
		 crp_l1 = lees_txt( filewijzer ++ );

		 readbuffer[nreadbuffer ] = (char) crp_l1;
		 crp_fread++;

		 crp_l2 = lees_txt( filewijzer ++);
		 crp_fread++;
		 ...
		 readbuffer[nreadbuffer + 1 ] = (char) crp_l2;
		 ...
		 switch (crp_l1) {
		    ...
		    case 'f' :
			  ...
			  case '8' : case '9' : case 'a' : case 'b' :
			  case 'c' : case 'd' : case 'e' : case 'f' :
			     vall = 16*alphahex( crp_l1 ) + alphahex( crp_l2 );
			     readbuffer[ nreadbuffer - 1 ] = (char) vall ;

			     /* hier stond '^'
				nreadbuffer hoeft niet te worden opgehoogd
				de wel in readbuffer geschreven bytes
				worden later overschreven
			      */
			     break;
			  default : /* foutje */
			     vall = 0;
...
void leesregelx()
{
	   ...
	   switch ( crp_l ) {
		 ...
		 switch ( crp_l1) {
		    ...
		    case 'f' :
		      crp_l2 = lees_txt( filewijzer );
		      filewijzer ++ ;
		      readbuffer[nreadbuffer-1] =
			  16*alphahex( crp_l1 ) + alphahex( crp_l2 );
   ...
   */
   /*
     hier readbuffer door lus_eerst() halen:
     veranderen van de accenten naar asc-II code...
    */
...
#include <c:\qc2\pools3\monoin0E.c>
/* testzoek5 */
