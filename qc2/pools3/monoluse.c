...
/*
   ...
   variable EOF_f is changed to 1, when code ^EF is found
...
int  lus( char ctest )
{
   ...
   switch ( ctest )
   {
		...
		*/
	    ...
	    case '=' :  /* @=n -> add n half squares alphahex... */
	       ...
	       if ( ( line_data.wsum +
		      ...
		      > central.inchwidth ) {
		  verschil = central.inchwidth - line_data.wsum;
	    ...
	    case 'C' :  /* end line */
	       ...
	       /*
	       if (verschil < 0 ) {
		  printf("Verschil negatief: %8.5f line %8.5f used %8.5f ",
			  verschil, central.inchwidth , line_data.wsum);
		  if (getchar()=='#') exit(1);
	       }
	       */
	 ...
	 /*
	    hier moet de uitvulcode voor de spaties komen,
	    die pas later wordt ingevuld als de regel klaar is

	    cop[ncop++] = 0;
	    cop[ncop++] = 0x24; / * S 0075 * /
	    cop[ncop++] = 0;    / *        * /
	    cop[ncop++] = 0;
	    cop[ncop++] = 0;
	    cop[ncop++] = 0x20; / *  S     * /
