...
{
    int t;
    t = (mcx[1] == 0x80 && mcx[3] == 0x02);
    return (t);
}
int test2_G14()
{
    int t;
    t = (mono.code[1] == 0x80 && mono.code[3] == 0x02);
    return (t);
}
...
int test_GS12()
{
    ...
    if (mono.code[1] == 0xa0 ) { /* GS */
       switch (mono.code[2] ) {
	  case 0x40 : t = 1; break; /* 1 */
	  case 0x20 : t = 1; break; /* 2 */
       }
    }
    return ( t == 1);
}
...
int test_O15()
{
    ...
    t += mcx[0]; t += mcx[1];
    t += mcx[2]; t += mcx[3];
    if (mono.code[3] & 0x80 ) t++;
    if (mono.code[1] & 0x80 ) t++;
...
void zenden_codes()
{
     ...
     */
     if (mcx[0] & 0x01 ) mcx[0] |=0x80;
     /* O aanzetten, dit het H-ventiel wil niet werken.... */

     for (ziii=0;ziii<4;ziii++) mono.code[ziii] = mcx[ziii] ;
     if ( test_k() ) {
	mcx[1] |= 0x20;
     }
