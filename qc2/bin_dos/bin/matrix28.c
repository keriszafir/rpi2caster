...
128 € 200  144  220  160   240  176 ° 260 192 À 280 208 Ð 300 224 à 320  240 ð
129  201  145 ‘ 221  161 ¡ 241  177 ±     193 Á     209 Ñ     225 á 321  241 ñ
130 ‚ 202  146 ’ 222  162 ¢ 242  178 ²     194 Â     210 Ò     226 â 322  242 ò
131 ƒ 203  147 “ 223  163 £ 243  179 ³     195 Ã     211 Ó     227 ã 323  243 ó
132 „ 204  148 ” 224  164 ¤ 244  180 ´     196 Ä     212 Ô     228 ä 324  244 ô
133 … 205  149 • 225  165 ¥ 245  181 µ     197 Å     213 Õ     229 å 325  245 õ
134 † 206  150 – 226  166 ¦ 246  182 ¶     198 Æ     214 Ö     230 æ 326  246 ö
135 ‡ 207  151 — 227  167 § 247  183 ·     199 Ç     215 ×     231 ç 327  247 ÷
136 ˆ 210  152 ˜ 230  168 ¨ 250  184 ¸     200 È     216 Ø     232 è 330  248 ø
137 ‰ 211  153 ™ 231  169 © 251  185 ¹     201 É     217 Ù     233 é 331  249 ù
138 Š 212  154 š 232  170 ª 252  186 º     202 Ê     218 Ú     234 ê 332  250 ú
139 ‹ 213  155 › 233  171 « 253  187 »     203 Ë     219 Û     235 ë 333  251 û
140 Œ 214  156 œ 234  172 ¬ 254  188 ¼     204 Ì     220 Ü     236 ì 334  252 ü
141  215  157  235  173 ­ 255  189 ½     205 Í     221 Ý     237 í 335  253 ý
142 Ž 216  158 ž 236  174 ® 256  190 ¾     206 Î     222 Þ     238 î 336  254 þ
143  217  159 Ÿ 237  175 ¯ 257  191 ¿     207 Ï     223 ß     239 ï 337  255
...
char r_eading()
{
    ...
    *fp = ( fpos_t ) ( p ) ;
    ...
    print_at(11,10,"wig   ");
    for (i=0;i<16;i++)
       printf("%2d ",cdata.wedge[i]);
    print_at(12,10,"corps ");
    for (i=0;i<10;i++)
       printf("%4d ",cdata.corps[i]);
    print_at(13,10,"set   ");
    for (i=0;i<10;i++)
       printf("%4d ",cdata.csets[i]);
    for (i=0;i<16;i++)
	    wig[i] = cdata.wedge[i];
...
void mainx()
{
    ...
    unsigned char tc[16],ss;

    int size ;

    /********** program text of routine **********************/
    /*
    size = (int) recsize;
    printf(" recsize %2d size %2d ",recsize,size );
    size = (int) recs2;
    printf(" recsize %2d size %2d ",recs2 ,size );
      */
    ...
    /* pri_num(123,5); */
    for (i=0;i< 5; i++) {
       print_at(2,2,"geef een string ");
       lt = get_line();

       print_at(4,2,"  terug = ");
       printf("%2d",lt);
       print_at(5,2,"string gelezen:");
       _outtext(r_buff);
       ontsnap("test");
    }
    ontsnap("stoppen");

    printf(" kolom = %3d ",askcol());

    nrows = 15;
    cls();
    printf(" rij = %3d ",askrow());
    getchar();
    ...
    ontsnap("terug van askrow experiment ");
  ...
  /*  print_at(7,20,"naam =");
    print_at(7,30,namestr);
    print_at(8,0,"  "); */

    do {
       print_at(8,20,"number of rows 15/16 ");
       get_line();
       nrows = atoi (r_buff);
    }
      while ( (nrows <15) || ( nrows >16) );
    ...
    for (i=0;i<=nrows-1;i++) {
      ...
      printf("%2d",iwig);
       ...
       /* lees wig[i] */
       /* test niet kleiner en niet te groot */
    }
    print_at(14,20,"                       ");
	...
	for (j=0;j<= 16  ;j++) {
	    ...
	    print_at(13,20,"row "); printf("%2d",i+1);
	    print_at(13,28,"col ");
	    pri_coln(j);
	    ...
	    for (k=0;k<3;k++)  matrix[i+j*16].lig[k]='\0';
	    for (k=0;k<lt-1;k++) {
	       matrix[i+j*16].lig[k] = r_buff[k];
	    }
	    if (lt == 3) {
	      convert( &matrix[i+j*17] );
	    }

	    matrix[i+j*16].srt = lees_srt( matrix[i+j*16].lig );
    ...
    for (recseek = 0; recseek <= aantal_records -1;
		      recseek ++ ){
	    ...
	    *fp = ( fpos_t ) ( p ) ;
	    ...
	    print_at(11,10,"lig = ");
	    for (j=0;j<3;j++)
	       printf("%1c",matrix[i].lig[j]);
	    print_at(12,10,"srt   ");
	    switch (matrix[i].w){
	       case 0: printf("romein     "); break;
	       case 1: printf("italic     "); break;
	       case 2: printf("small caps "); break;
	       case 3: printf("bold       "); break;
	    }
	    print_at(13,10,"width : ");
	    printf("%2d ",matrix[i].w);
