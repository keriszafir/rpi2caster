...
void spaces()
{
    ...
    do {
	  ...
	  print_at(10,1,"");
	  printf("Width of the space in points ");
	  /* get_line(); */
	  ...
	  while (w_sp < 2  || w_sp > 12. );



       do {
	  printf("\n");
	  printf("How many spaces to be cast   ");
	  number = p_atoi( get_line() );
       }
	  while (number <= -1 );
	  ...
	  printf("row %2d units %2d ",row+1, wedge[row]);
	  printf("wedge  = %10.7f \n",wigg);
	  printf("Space  = %10.7f \n",sp_width);
	  printf("Delta  = %10.7f \n",delta_space);
	  delta_space *= 2000;
	  ...
	  for (i=0;i<4;i++) mcx[i]=pump_off[i];
	  setrow(spu1);
	  f1();
	  if ( interf_aan ) zenden_codes();
