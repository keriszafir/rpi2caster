...
void apart()
{
    ...
    setrow(u1);
    for (tj=0;tj<4;tj++) wig_75[tj] = mcx[tj];


    for (tj=0;tj<4;tj++) mcx[tj]=0;
    mcx[3] |= 0x01; /* 0005 */
    setrow(u2);
    for (tj=0;tj<4;tj++) wig_05[tj] = mcx[tj];

    mcx[1] |= 0x04; /* 0075 */
    for (tj=0;tj<4;tj++) galley[tj] = mcx[tj];

    do
    {
       aantal = ask_number(10, 300);
       printf("Casting the chars          : \n");
       printf("Put on the motor           : ");
       getchar();

       for (tj=0;tj<4;tj++) mcx[tj]=wig_05[tj];
       f1();
       ...
       for (tj=0;tj<4;tj++) mcx[tj]=wig_75[tj];
       f1();
       if ( interf_aan ) zenden_codes();

       do {
	  a2 = (aantal > 10 ) ? 10 : aantal;
	  for ( ti =0; ti < a2 ; ti++)
	  {
	     for (tj=0;tj<4;tj++) mcx[tj]=l[tj];
	     printf("Left to be cast : %3d \n",aantal --);
	     f1();
	     if ( interf_aan ) zenden_codes();
	  ...
	  for (tj=0;tj<4;tj++) mcx[tj]=galley[tj];
	  f1();  /* eject line */
	  if ( interf_aan ) zenden_codes();
...
void apart2()
{
    ...
    for (i=0;i<4;i++) l[i]=0;
    /*   ONMLKJIH GFSEDCgB A1234567 89abcdek
...
int ask_number(int min, int max)
{
    ...
    do {
       do {
	  printf("how many characters to be cast    ? ");
	  while ( get_line() < 1);
...
void cases()
{
    ...
    printf("Casting Bembo 16 for cases \n\n");
    ...
    do
    {
	  ...
	  c_a[i]=0;
       ...
       for ( char_nr = 0; char_nr <  4 ; char_nr++ )
       {
	  for (i=0;i<4;i++) l[i]=0;
	  row = read_row()-1;
	  col = read_col()  ;
	  ...
	  */
	  ...
	  aantal[char_nr] = ask_number( 10, 300 );

	  c_totaal += aantal[char_nr];
       }

       printf("Put the die-case in the machine   : ");
       if (getchar()=='#') exit(1);
       printf("Start the motor of the machine    : ");
	  ...
	  for ( char_nr = 0; char_nr < 4 ; char_nr++)
	  {
	      ...
	      /*
	      printf(" aantallen %3d %3d %3d %3d ",
		  aantal[0],aantal[1],aantal[2],aantal[3]);
	      printf(" totaal %5d \n",aantal[0]+aantal[1]
			+aantal[2]+aantal[3]);
	       */
		 ...
		 width = c_w[char_nr];
		 for (i=0;i<4;i++) l[i]=c_l [char_nr] [i];
		 ...
		 for (case_j = 0 ; case_j < number; case_j++) {
		     ...
		     c_a[char_nr] ++;
		     printf("Cast %4d of character %3d  \n",
				     c_a[char_nr],char_nr+1);
		     if ( width >= 15 )
		     {
    ...
    */
    ...
    do {
	  ...
	  case 'y' :
		...
		while ( caster != 'c' && caster != 'k' && caster != ' ');
	     cases();
	     break;
       }
    }
