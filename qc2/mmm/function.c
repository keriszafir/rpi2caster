   ...
   void scherm2()
   {
	   ...
	   case SHIFT : nrows = RIJAANTAL;    break;
	   case MNH   : nrows = RIJAANTAL;    break;
	   case MNK   : nrows = RIJAANTAL;    break;
       }
	*/

       nrows = central.syst < MNH ? RIJAANTAL - 1: RIJAANTAL ;
	...
	*/
	   ...
	   print_at(scherm_i+4,1,"");  printf("%2d",wig[scherm_i] ) ;
	   print_at(scherm_i+4,78,""); printf("%2d",scherm_i + 1 );
	   /* if (scherm_i > 18)  ce(); */
       }
   }
   ...
   /*
	 ...
	 scherm3();
      ...
      version 30march 2006 : variable with central.syst
   ...
   char displaym()
   {
	 ...
	 printf("%1c",dis_c);
      dism = central.syst < MNH ? 255 : 272 ;

      for (dis_i=0; dis_i< dism ; dis_i++){ /* central.syst */
	 dis_mm = matrix[dis_i];
	 pri_lig( & dis_mm );
      }
   ...
   unsigned char get__srt(int row, int col )
   {
	  ...
	  if (c==0) getch();
	  switch ( c ) {
	     case 'r' : c = '0'; break; /* roman */
	     case 'i' : c = '1'; break; /* italic */
	     case 's' : c = '2'; break; /* small capitals */
	     case 'b' : c = '3'; break; /* bold  */
	     case 'u' : c = '4'; break; /* superior  */
	     case 'd' : c = '5'; break; /* inferior */
	  }
	  if (c<'0' || c>'5') {
	     print_at(row,col," ");
	     print_at(row,col,"");
   ...
   int lig__get(int row, int col)
   {
	  ...
	  switch ( c ) {
		...
		if (nlineb > 0) {
		   switch (line_buffer[nlineb-1] ){
		      case 'd': /* 208 Ð d0 320 */
			  line_buffer[nlineb-1]=0xd0; c2 =1; break;
		      case 'D': /* 209 Ñ d1 321 */
			  line_buffer[nlineb-1]=0xd1; c2 =1; break;
		      default : break;
		   }
		}
		break;
		...
		if (nlineb > 0) {
		   switch (line_buffer[nlineb-1] ){
		       case '>': line_buffer[nlineb-1]=0xae; c2=1; break;
		       default : break;
		   }
		}
		break;
		...
		if (nlineb > 0) {
		   switch (line_buffer[nlineb-1] ){
		       case '<': line_buffer[nlineb-1]=0xaf; c2=1; break;
		       default : break;
		   }
		}
		break;
		...
		if (nlineb == 1 ) {
		   switch (line_buffer[nlineb-1] ){
		       case 'A': line_buffer[nlineb-1]=0x92; c2 = 1; break;
		       default : break;
		   }
		}
		break;
		...
		if (nlineb == 1 ) {
		   switch (line_buffer[nlineb-1] ){
		       case 'a': line_buffer[nlineb-1]=0x91; c2 = 0; break;
		       default : break;
		   }
		}
		break;
		...
		if (nlineb > 0) {
		   switch (line_buffer[nlineb-1] ){
		       case 's': line_buffer[nlineb-1]=0xe1; c2=1; break;
		       default : break;
		   }
		}
		break;
		...
		if (nlineb > 0) {
		   switch (line_buffer[nlineb-1] ){
		       case 'c': line_buffer[nlineb-1]=0x87; c2=1; break;
		       case 'C': line_buffer[nlineb-1]=0x80; c2=1; break;
		       default : break;
		   }
		}
		break;
		...
		if ( nlineb > 0 ) {
		   switch (line_buffer[nlineb-1] ){
		       case 'n': line_buffer[nlineb-1]=0xa4; c2=1; break;
		       case 'N': line_buffer[nlineb-1]=0xa5; c2=1; break;
		       case 'a': line_buffer[nlineb-1]=0xc6; c2=1; break;
		       case 'A': line_buffer[nlineb-1]=0xc7; c2=1; break;
		       default : break;
		   }
		}
		break;
		...
		if (nlineb > 0 ) {
		   switch (line_buffer[nlineb-1] ){
		      case 'a' : line_buffer[nlineb-1]=0x84; c2=1; break;
		      case 'e' : line_buffer[nlineb-1]=0x89; c2=1; break;
		      case 'i' : line_buffer[nlineb-1]=0x8b; c2=1; break;
		      case 'o' : line_buffer[nlineb-1]=0x94; c2=1; break;
		      case 'u' : line_buffer[nlineb-1]=0x81; c2=1; break;
		      case 'A' : line_buffer[nlineb-1]=0x8e; c2=1; break;
		      case 'E' : line_buffer[nlineb-1]=0xd3; c2=1; break;
		      case 'I' : line_buffer[nlineb-1]=0xd8; c2=1; break;
		      case 'O' : line_buffer[nlineb-1]=0x99; c2=1; break;
		      case 'U' : line_buffer[nlineb-1]=0x9a; c2=1; break;
		      default  : break;
		   }
		}
		break ;
		...
		if (nlineb > 0 ) {
		   switch (line_buffer[nlineb-1] ){
		       case 'a' : line_buffer[nlineb-1]=0x85; c2=1; break;
		       case 'e' : line_buffer[nlineb-1]=0x8a; c2=1; break;
		       case 'i' : line_buffer[nlineb-1]=0x8d; c2=1; break;
		       case 'o' : line_buffer[nlineb-1]=0x95; c2=1; break;
		       case 'u' : line_buffer[nlineb-1]=0x97; c2=1; break;
		       case 'A' : line_buffer[nlineb-1]=0xb7; c2=1; break;
		       case 'E' : line_buffer[nlineb-1]=0xd4; c2=1; break;
		       case 'I' : line_buffer[nlineb-1]=0xde; c2=1; break;
		       case 'O' : line_buffer[nlineb-1]=0xe3; c2=1; break;
		       case 'U' : line_buffer[nlineb-1]=0xeb; c2=1; break;
		       default  : break;
		   }
		}
		break ;
		...
		if (nlineb > 0 ) {
		   switch (line_buffer[nlineb-1] ){
		       case 'y' : line_buffer[nlineb-1]=0xec; c2=1; break;
		       case 'Y' : line_buffer[nlineb-1]=0xed; c2=1; break;
		       case 'a' : line_buffer[nlineb-1]=0xa0; c2=1; break;
		       case 'e' : line_buffer[nlineb-1]=0x82; c2=1; break;
		       case 'i' : line_buffer[nlineb-1]=0xa1; c2=1; break;
		       case 'o' : line_buffer[nlineb-1]=0xa2; c2=1; break;
		       case 'u' : line_buffer[nlineb-1]=0xa3; c2=1; break;
		       case 'A' : line_buffer[nlineb-1]=0xb5; c2=1; break;
		       case 'E' : line_buffer[nlineb-1]=0x90; c2=1; break;
		       case 'I' : line_buffer[nlineb-1]=0xd6; c2=1; break;
		       case 'O' : line_buffer[nlineb-1]=0xe0; c2=1; break;
		       case 'U' : line_buffer[nlineb-1]=0xe9; c2=1; break;
		       default  : break;
		   }
		}
		break ;
		...
		if (nlineb > 0 ) {
		   switch (line_buffer[nlineb-1] ){
		       case 'a' : line_buffer[nlineb-1]=0x83; c2=1; break;
		       case 'e' : line_buffer[nlineb-1]=0x88; c2=1; break;
		       case 'i' : line_buffer[nlineb-1]=0x8c; c2=1; break;
		       case 'o' : line_buffer[nlineb-1]=0x93; c2=1; break;
		       case 'u' : line_buffer[nlineb-1]=0x96; c2=1; break;
		       case 'A' : line_buffer[nlineb-1]=0xb6; c2=1; break;
		       case 'E' : line_buffer[nlineb-1]=0xd2; c2=1; break;
		       case 'I' : line_buffer[nlineb-1]=0xd7; c2=1; break;
		       case 'O' : line_buffer[nlineb-1]=0xe2; c2=1; break;
		       case 'U' : line_buffer[nlineb-1]=0xea; c2=1; break;
		       default  : break;
		   }
		}
		break ;
	  ...
	  while (nlineb < 4 && c != 13 );
       ...
       for ( nn = 0,c1= 0; nn < 3 ; nn++)     /* 3 lines added 3-3-2006 */
	  if (line_buffer[nn] != '\0') c1++;
       nlineb = c1;
       if (nlineb == 3 ) {
	  if ( line_buffer[1] == '/' ) {
   ...
   void read_outputname(int s )
   {
	 ...
	 switch ( s ) {
	    case 1 : strcpy( ext, "mat"); break;
	    case 2 : strcpy( ext, "txt"); break;
	 }
   ...
   */
   ...
   void read_mat( )
   {
	 ...
	 readbuffer[tst_i] = '\0'; /* at the beginning this is empty */
      ...
      inpathtext[1]='\0';
      cls();
      print_at(3,0,"Give name matrix file ");
      read_inputname();
