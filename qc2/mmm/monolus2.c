...
void add_wsum ( int w )
{
    line_data.wsum += ((float) ( w * central.set) ) / 5184.;
}
   ...
   switch ( ctest )
   {
	 ...
	 switch ( luscy )   /* readbuffer[t3j+1] */
	 {
	    ...
	    case 'r' : /* repeat signal for "kerning" */
	       ...
	       break;
	    case '#' :  /* @#n add n times wig[14]=18 ? units squares alpha-hex .... */
		  ...
		  if (lusaddsqrs != 0 ) {
	       ...
	       /* O-15 = default 18 unit space  wig[14] differs*/
	       break;
	    case '=' :  /* @=n -> add n half squares alphahex... */

	       if ( ( line_data.wsum +
		      ((float) ( alphahex(luscx) * wig[4] * central.set ) ) / 5184.)
		      > central.inchwidth ) {
